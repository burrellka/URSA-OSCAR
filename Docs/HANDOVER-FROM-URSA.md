...
3. **Google Health Connect vs. Google Health Web API.** The Sept 2026 Fitbit Web API shutdown migrates to Health Connect — but Health Connect is Android-native. The server-side Health REST API is the more likely target for a self-hosted analytics deployment. Confirm WHICH API before scoping.

...
URSA-OSCAR's MCP container handles `X-Forwarded-Proto` correctly out of the box as of 1.1.17 — but the proxy still has to send it.
...
```yaml
  ursa-oscar-mcp:
    image: brain40/ursa-oscar-mcp:1.1.17
...
| Tools list returns empty | Tools registered against wrong module | Should be impossible with 1.1.17 — file a bug if it happens |

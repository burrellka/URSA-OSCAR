...
- **"image not found" or "manifest unknown"** → You may have an old or mistyped image tag in the compose file. Make sure all four image lines say `:1.1.9` (or whatever version is current; check the [latest release](https://github.com/burrellka/URSA-OSCAR/releases) if unsure).
...
```powershell
Set-Location C:\URSA-OSCAR
# Edit docker-compose.yml — change the four "image: brain40/ursa-oscar-*:1.1.9"
...
```powershell
docker image rm brain40/ursa-oscar-api:1.1.9 brain40/ursa-oscar-web:1.1.9 brain40/ursa-oscar-watcher:1.1.9
```

...
Operators typically take `docker-compose.production.yml` as the starting point and edit the bind-mount paths + tunnel hostname to match their setup. The image tags are pinned (currently 1.1.3); pulling a new release means editing the file to bump the tags.
